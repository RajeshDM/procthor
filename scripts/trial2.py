...
from procthor.utils import is_equal, load_scene_json_file
...
class McsHumanControlEnv():
    def __init__(self,  **args):
        ...
        scene = args.get('scene_json', None)
    ...
    def step(self, action_str, **args):
        ...
        if "Move" in action_str:
            self.step_output = self.controller.step(action=action_str,**args)
...
if __name__ == '__main__':
    ...
    #scene_json = dataset['train'][0]
    path = "generated_data/temp.json"
    scene_json = load_scene_json_file(path)

    env = McsHumanControlEnv(task="interaction_scenes", scene_type="eval3", start_scene_number=start_scene_number, frame_collector=None,scene_json=scene_json)
    #controller = Controller()
    #env.reset()
    standard_rotation = 10
    standard_movement = 0.25

    while True:
        action = input("Enter Action: ")
        if action == "w":
            env.step("MoveAhead",moveMagnitude=standard_movement)
        elif action == "s":
            env.step("MoveBack",moveMagnitude=standard_movement)
        elif action == "a":
            env.step("MoveLeft",moveMagnitude=standard_movement)
        elif action == "d":
            env.step("MoveRight",moveMagnitude=standard_movement)
        elif action == "q":
            # rt = input("RotateLeft! Enter the rotation: ")
            rt = 10
            #env.step("RotateLeft", rotation=-float(rt))
            env.step('RotateLeft',degrees=rt)
        elif action == "e":
            # rt = input("RotateRight! Enter the rotation: ")
            rt = 10
            #env.step("RotateRight", rotation=float(rt))
            env.step('RotateRight',degrees=rt)
        elif action == "r":
