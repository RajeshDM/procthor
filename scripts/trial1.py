...
commands_dict_list =[
{'name': 'Initialize', 'key': '0', 'desc': 'Initialization'},
{'name': 'CloseObject', 'key': '1', 'desc': 'Close a nearby object. (objectId=string, amount=float (default:1), objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'DropObject', 'key': '2', 'desc': 'Drop an object you are holding. (objectId=string)'},
{'name': 'MoveAhead', 'key': 'w', 'desc': 'Move yourself ahead based on your current view.'},
{'name': 'MoveBack', 'key': 's', 'desc': 'Move yourself back based on your current view.'},
{'name': 'MoveLeft', 'key': 'a', 'desc': 'Move yourself to your left based on your current view.'},
{'name': 'MoveRight', 'key': 'd', 'desc': 'Move yourself to your right based on your current view.'},
{'name': 'OpenObject', 'key': '3', 'desc': 'Open a nearby object. (objectId=string, amount=float (default:1), objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'PickupObject', 'key': '4', 'desc': 'Pickup a nearby object and hold it in your hand. (objectId=string, objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'PullObject', 'key': '5', 'desc': 'Pull a nearby object. (objectId=string, force=float (default:0.5), objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'PushObject', 'key': '6', 'desc': 'Push a nearby object. (objectId=string, force=float (default:0.5), objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'PutObject', 'key': '7', 'desc': 'Place an object you are holding into/onto a nearby receptacle object. (objectId=string, receptacleObjectId=string, receptacleObjectImageCoordsX=float, receptacleObjectImageCoordsY=float)'},
{'name': 'TorqueObject', 'key': '8', 'desc': 'Apply torque to a nearby object. (objectId=string, force=float(default:0.5), objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'RotateObject', 'key': '9', 'desc': 'Apply a rotation of 5 degrees to a nearby object. (objectId=string, clockwise=bool(default:True), objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'MoveObject', 'key': 'm', 'desc': 'Apply a movement of 0.1 meters to a nearby object. (objectId=string, lateral=int(default:0), straight=int(default:1), objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'InteractWithAgent', 'key': 't', 'desc': 'Interact with an agent. If that agent has an object, it will hold out the object for you to pickup; otherwise, the agent will look sad.(objectId=string, objectImageCoordsX=float, objectImageCoordsY=float)'},
{'name': 'LookUp', 'key': 'i', 'desc': 'Rotate your view up (subtract 10 degrees from head tilt).'},
{'name': 'LookDown', 'key': 'k', 'desc': 'Rotate your view down (add 10 degrees to head tilt).'},
{'name': 'RotateLeft', 'key': 'j', 'desc': 'Rotate your view left by 10 degrees.'},
{'name': 'RotateRight', 'key': 'l', 'desc': 'Rotate your view right by 10 degrees.'},
{'name': 'EndHabituation', 'key': 'h', 'desc': 'Ends a habituation trial for the scene by blanking the screen for one action (and teleporting the agent if needed). Sometimes needed depending on the task type.'},
{'name': 'EndScene', 'key': 'q', 'desc': 'There is no action available and end_scene needs to be called.'},
{'name': 'Pass', 'key': ' ', 'desc': 'Do nothing. (no params)'}
        ]
...
class HumanInputShell(cmd.Cmd):
    ...
    def do_reset(self, args=None):
        ...
        self.previous_output = (self.controller).step("Pass")
        print (self.previous_output.__dict__.keys())
        exit()
        self.show_available_actions(self.previous_output)
...
def build_commands():
    ...
    for action in commands_dict_list:
        #commands.append(Command(action.value,
        #                        action.key,
        #                        action.desc))
        #print (str(commands[-1].__dict__)+ ',')
        commands.append(Command(action['name'],
                                action['key'],
                                action['desc']))
